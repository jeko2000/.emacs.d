...
(use-package eglot
  ...
  (define-key eglot-mode-map (kbd "C-c o r") 'eglot-rename)
  (define-key eglot-mode-map (kbd "C-c o i") 'eglot-code-action-organize-imports)
  (define-key eglot-mode-map (kbd "C-S-o") 'eglot-code-action-organize-imports))
#+end_src
** Language Specific
*** Eshell
...
(use-package eglot
  :config
  (let ((lombok-path (expand-file-name "~/prog/java/lib/lombok.jar")))
    (when (file-exists-p lombok-path)
      (let ((jvm-args (concat " " lombok-path
                              " --jvm-arg=-Xbootclasspath/a:" lombok-path)))
        (add-to-list 'eglot-server-programs
                     `(java-mode . ("jdtls"
                                    ,(concat "--jvm-arg=-javaagent:" lombok-path))))))))
