...
#+END_SRC
Then, we borrow some code from [[https://www.emacswiki.org/emacs/FlySpell#toc5][the wiki]] to make it easier to cycle
between languages
#+BEGIN_SRC emacs-lisp :tangle yes
(let ((langs '("en_US" "castellano8")))
...
#+END_SRC
*** Evaluation
The following code will evaluate a sexp and replace it with its value.
We will bind it to "C-c e" as recommended.
#+BEGIN_SRC emacs-lisp :tangle yes
...
#+BEGIN_SRC emacs-lisp :tangle yes
(setq gnus-config-file (expand-file-name "settings/gnus-init.el" user-emacs-directory))
...
(setq user-full-name "Johnny Ruiz"
      user-mail-address "kubb18@gmail.com")
#+END_SRC
