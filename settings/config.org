...
(use-package lsp-mode
  ...
  (defun jr/lsp-mode-before-save-hook ()
  ...
  (add-hook 'before-save-hook #'jr/lsp-mode-before-save-hook))
...
(use-package lsp-python-ms
  ...
  (defun jr/python-mode-hook ()
    ...
    (lsp))
  :hook (python-mode . jr/python-mode-hook)
  :config
...
(use-package scala-mode
  :mode "\\.s\\(cala\\|bt\\)$"
  :init
  (defvar jr/lsp-mode-hooks/scala-mode nil
    "Hooks to run when lsp is enabled on a scala major-mode buffers.")

  (defun jr/lsp-mode-run-hooks/scala-mode ()
    (when (eq major-mode 'scala-mode)
      (run-hooks 'jr/lsp-mode-hooks/scala-mode)))
  :hook ((scala-mode . lsp)
         (lsp-mode . jr/lsp-mode-run-hooks/scala-mode)
         (before-save . jr/scala-mode-before-save-hook)))
...
(use-package sbt-mode
  ...
  (when (boundp 'jr/lsp-mode-hooks/scala-mode)
    (add-hook 'jr/lsp-mode-hooks/scala-mode #'sbt-start))

  (defun jr/sbt-mode-calculate-run-main-command ()
    "Return the runMain command associated with the current source file."
    (let* ((package (mapconcat 'identity (cdddr (member "src" (split-string default-directory "/" t))) "."))
           (object (file-name-sans-extension (buffer-name))))
      (format "runMain %s.%s" package object)))

  (defun jr/sbt-mode-run-current-buffer ()
    (interactive)
    (sbt-command (jr/sbt-mode-calculate-run-main-command)))

  (define-key scala-mode-map (kbd "C-c C-l C-c") #'jr/sbt-mode-run-current-buffer)

  (defun jr/sbt-mode-after-save-hook ()
    (when (eq major-mode 'scala-mode)
      (sbt-command (jr/sbt-mode-calculate-run-main-command))))
  (add-hook 'after-save-hook #'jr/sbt-mode-after-save-hook))
    #+end_src
**** Build metals-emacs
...
** Copy current file
#+begin_src emacs-lisp :tangle yes
(defun jr/kill-buffer-file-name ()
  (interactive)
