...
(use-package treemacs
  :init
  (defun jr/treemacs-mode-hook ()
    (treemacs-git-mode 'simple))
  :bind (:map global-map
              ("<f8> <f8>" . treemacs)
              ("<f8> 0" . treemacs-select-window)
              ("<f8> B" . treemacs-bookmark)
              ("<f8> C-x C-f" . treemacs-find-file))
  :hook ((treemacs-mode . treemacs-tag-follow-mode)
         (treemacs-mode . treemacs-fringe-indicator-mode)
         (treemacs-mode . treemacs-filewatch-mode))
  :config
  (define-key treemacs-mode-map [mouse-1] #'treemacs-single-click-expand-action)
  (treemacs-resize-icons 20)
  (setq treemacs-tag-follow-delay 1
